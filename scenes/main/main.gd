extends Node3D
## Игровая сессия (один прогон ОДНОГО ЭПИЗОДА). Владеет последовательностью
## уровней эпизода, игроком, HUD и звуком; свапает уровни НА МЕСТЕ (игрок/HUD/
## CombatAudio переживают переход — эмбиент не перезапускается, лоадаут
## переносится между уровнями эпизода).
##
## Сброс лоадаута МЕЖДУ эпизодами — «полная классика» — получается сам собой:
## конец эпизода возвращает в главное меню, роутер сносит сессию, следующий
## эпизод = новая сессия со свежим игроком (@export-дефолты: кулак + пистолет).
##
...
## Какой эпизод играем. Роутер задаёт выбранный игроком Episode ДО add_child;
## значение в инспекторе main.tscn — отладочный эпизод для запуска сцены по F6.
...
## Уровень пройден, впереди ещё есть уровни эпизода. Роутер покажет промежуточный
## экран; по "Дальше" вызовет advance_level().
signal level_completed
## Пройден последний уровень эпизода. Роутер покажет концовку эпизода.
signal episode_completed

# Индекс текущего уровня в episode.levels. Прогресс эпизода живёт ЗДЕСЬ, в сессии
# (состояние — в сцене, не в роутере и не в autoload). Пара (эпизод, уровень) для
# облачного резюма Этапа 4: эпизод выбирает роутер, уровень — set_start_level
# до добавления сессии в дерево.
...
func _ready() -> void:
	if episode == null or episode.levels.is_empty() \
			or player_scene == null or hud_scene == null:
		push_error("Main: нужен episode (с непустым levels), player_scene и hud_scene.")
		return
...
# Стартовый индекс уровня ВНУТРИ эпизода (для будущего облачного резюма, Этап 4).
...
# Команда роутера: следующий уровень эпизода (после промежуточного экрана). Игрок
# и его лоадаут сохраняются (continuous-режим — перенос прогресса между уровнями).
func advance_level() -> void:
	_level_index += 1
	if _level_index >= episode.levels.size():
		_level_index = episode.levels.size() - 1
		return
	_load_level(false)
...
func _load_level(fresh_player: bool) -> void:
	...
	_level = episode.levels[_level_index].instantiate()
	add_child(_level)
...
# Игрок дошёл до выхода. Решаем: в эпизоде ещё есть уровни → промежуточный экран,
# иначе → концовка эпизода. Сам свап делаем по команде роутера (advance_level).
func _on_level_exit(_exit: LevelExit) -> void:
	if _level_index + 1 < episode.levels.size():
