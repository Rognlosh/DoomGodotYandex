...
func _init() -> void:
	# Ближний бой — ОДИН кадр по дизайну (замах кодом), а дефолт базы (4) заточен
	# под стрипы стволов. Переопределяем, чтобы забытый инспектор не резал текстуру
	# на четвертинки. Значение из сцены (если задано) применится позже и победит.
	sprite_frames = 1
